...
RUN set -ex ;\
    ...
    rm -rf /var/cache/apk/* ;\
    find /usr/share/ca-certificates/mozilla/ -name "*.crt" -exec keytool -import -trustcacerts -keystore /usr/lib/jvm/java-1.8-openjdk/jre/lib/security/cacerts -storepass changeit -noprompt -file {} -alias {} \; ;\
...
RUN set -ex ;\
    unzip /tmp/sonar-scanner-cli-${SONAR_SCANNER_VERSION}.zip -d /usr/lib ;\
